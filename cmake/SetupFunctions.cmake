...
FUNCTION(LIBRARY_SETUP LIB_NAME)
  ...
  IF(${CMAKE_LIBRARY_TYPE} EQUAL "SHARED")
    SET(INST_DST_TYPE "LIBRARY")
  ELSE()
    SET(INST_DST_TYPE "ARCHIVE")
  ENDIF()
  
  INSTALL(TARGETS ${LIB_NAME} ${INST_DST_TYPE} DESTINATION ${MITSUBA_ADDONS_LIBRARY_DIRECTORY}
    COMPONENT lib)

  IF(EXISTS "${CMAKE_SOURCE_DIR}/inc/${LIB_NAME}" AND
      IS_DIRECTORY "${CMAKE_SOURCE_DIR}/inc/${LIB_NAME}")
    INSTALL(DIRECTORY ${CMAKE_SOURCE_DIR}/inc/${LIB_NAME} DESTINATION ${MITSUBA_ADDONS_HEADER_DIRECTORY}
      COMPONENT hdr)
  ENDIF()
ENDFUNCTION()

# PLUGIN_SETUP(NAME SOURCES .. [DEPENDENCIES ..] [DEBUG])

FUNCTION(PLUGIN_SETUP PLUGIN_NAME)
  SET(OARGS DEBUG)
  SET(SARGS)
  SET(MARGS SOURCES DEPENDENCIES)
  CMAKE_PARSE_ARGUMENTS(PLUGIN "${OARGS}" "${SARGS}" "${MARGS}" ${ARGN})
  
  IF(NOT PLUGIN_SOURCES)
    MESSAGE(SEND_ERROR "Error: PLUGIN_SETUP() called without any source files")
    RETURN()
  ENDIF()
  ...
  LIST(APPEND PLUGIN_SOURCES ${PRVHDRS})
  
  IF(PLUGIN_DEBUG OR VERBOSE)
    MESSAGE(STATUS "PLUGIN_SETUP(${PLUGIN_NAME}) variable setup:\n"
  ...
  SET_TARGET_PROPERTIES(${PLUGIN_NAME} PROPERTIES CMAKE_LIBRARY_OUTPUT_DIRECTORY
    ${CMAKE_PLUGIN_OUTPUT_DIRECTORY})

  SET_PROPERTY(TARGET ${PLUGIN} PROPERTY FOLDER "Plugins")
  SOURCE_GROUP(Source          REGULAR_EXPRESSION ".+\\.cpp")
  SOURCE_GROUP(Header\\Public  REGULAR_EXPRESSION ".+/inc/${PLUGIN_NAME}/.+\\.(hpp|inl)")
  SOURCE_GROUP(Header\\Private REGULAR_EXPRESSION ".+\\.hpp")
  
  TARGET_LINK_LIBRARIES(${PLUGIN_NAME} ${PLUGIN_DEPENDENCIES})  
  
  INSTALL(TARGETS ${PLUGIN_NAME} LIBRARY DESTINATION ${MITSUBA_ADDONS_PLUGIN_DIRECTORY}
    COMPONENT lib)
ENDFUNCTION()
...
FUNCTION(TEST_SETUP TST_PREFIX)
  ...
  IF(MITSUBA_ADDONS_BLD_UTEST)
    IF(MITSUBA_ADDONS_RUN_CTEST)
	...
	ADD_CUSTOM_TARGET(test_${TST_PREFIX} COMMAND ${CMAKE_CTEST_COMMAND} DEPENDS ${TST_EXES})
      ENDIF()
    ELSEIF(MITSUBA_ADDONS_RUN_UTEST)
      IF(TARGET test_${TST_PREFIX})
...
FUNCTION(APPLICATION_SETUP APP_NAME)
  ...
  INSTALL(TARGETS ${APP_NAME} RUNTIME DESTINATION ${MITSUBA_ADDONS_RUNTIME_DIRECTORY} COMPONENT app)
ENDFUNCTION()
